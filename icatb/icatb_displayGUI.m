...
if isfield(sesInfo, 'modality')
    if ~strcmpi(sesInfo.modality, 'fmri')
        error('Use EEGIFT toolbox to display EEG results');
    end
end
...
%try
    ...
    V = icatb_getVol(firstScan, fileNum);
    V.fname = maskImage;
    V.n(1) = 1;
    ...
    outputFiles = dispParameters.icaOutputFiles;
    ...
    for jj = 1 : numOfSets
        ...
        for kk = 1 : length(outputFiles(jj).ses)
            str = outputFiles(jj).ses(kk).name(1, :);
...
structFile = icatb_selectEntry('typeEntity', 'file', 'title', 'Select Structural File', 'filter', ...
    '*.img;*.nii', 'fileType', 'image', 'fileNumbers', 1);
...
try
    ...
    switch lower(displayMethod)
        case 'component explorer'
            ...
            P = dispParameters.icaOutputFiles(str2num(a)).ses(str2num(b)).name;
        ...
        case 'orthogonal viewer'
            ...
            %P = dispParameters.icaOutputFiles(str2num(a)).ses(str2num(b)).name(compNum,:);
            P = dispParameters.icaOutputFiles(str2num(a)).ses(str2num(b)).name;
            icaFiles = P;
        ...
        case 'subject component explorer'
            ...
            %numComp = length(dispParameters.icaOutputFiles);
            numComp = 0;
            ...
            for kk = 1:length(dispParameters.icaOutputFiles)
                for ii = 1:length(dispParameters.icaOutputFiles(kk).ses)
                    counter = counter + 1;
                    %P = dispParameters.icaOutputFiles(kk).ses(ii).name(compNum, :);
                    P = str2mat(dispParameters.icaOutputFiles(kk).ses(ii).name);
                    PFullFiles = P;
                    % get the files from the zip file
