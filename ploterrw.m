function [h,Options]=ploterrw(x,y,s,varargin)
...
if nargin<3
    s=[];
    if isscalar(y)
        y=permute(x,[y setdiff(1:ndims(x),y)]);
        x=[];
    end
end
...
if ~isempty(s) 
    ...
    s=[];
    elseif isscalar(s)
        y=permute(y,[s setdiff(1:ndims(y),s)]);
        s=[];
    end
end
Options.Color=get(gca, 'ColorOrder');
if Options.Color(2)~=1
    Options.Color([1 2 3],:)=Options.Color([1 3 2],:);
    set(gca, 'ColorOrder',Options.Color);
end
