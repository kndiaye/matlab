...
% FORMAT H = myspm_orthviews('Image',filename[,position])
...
% split colour table.
%
% myspm_orthviews('AddColouredBlobs',handle,XYZ,Z,mat,colour)
% Adds blobs from a pointlist to the image specified by the handle(s).
% handle   - image number to add blobs to
% XYZ      - blob voxel locations (currently in millimeters)
% Z        - blob voxel intensities
...
% blobs print well.
%
% myspm_orthviews('Register',hReg)
% See spm_XYZreg for more information.
%
% FORMAT myspm_orthviews('RemoveBlobs',handle)
...
% See spm_XYZreg for more information.
%
% myspm_orthviews('AddContext',handle)
% handle   - image number to add context menu to
%
% myspm_orthviews('RemoveContext',handle)
% handle   - image number to remove context menu from
%
% CONTEXT MENU
% myspm_orthviews offers many of its features in a context menu, which is
% accessible via the right mouse button in each displayed image.
%
% PLUGINS
% The display capabilities of myspm_orthviews can be extended with
% plugins. These are located in the myspm_orthviews subdirectory of the SPM
...
% myspm_orthviews('plugin_name', plugin_arguments). For detailed descriptions
% of each plugin see help myspm_orthviews/spm_ov_'plugin_name'.
%
%_______________________________________________________________________
% @(#)myspm_orthviews.m	2.38 John Ashburner, Matthew Brett, Tom Nichols and Volkmar Glauche 03/04/17
...
% vols cell array.  Handles returned by "myspm_orthviews('Image',.....)"
...
% of myspm_orthviews without the need to rewrite parts of it. Interaction
% between myspm_orthviews and plugins takes place
% a) at startup: The subfunction 'reset_st' looks for files with a name
%                spm_ov_PLUGINNAME.m in the directory 'SWD/myspm_orthviews'.
...
%                same way as in the redraw subfunction of myspm_orthviews.
...
case 'addtruecolourimage',
	% myspm_orthviews('Addtruecolourimage',handle,filename,colourmap,prop,mx,mn)
...
if ~isempty(h),
	...
	spm_XYZreg('Add2Reg',st.registry.hReg,st.registry.hMe, 'myspm_orthviews');
else,
...
DeleteFcn = ['myspm_orthviews(''Delete'',' num2str(ii) ');'];
V.ax = cell(3,1);
for i=1:3,
	ax = axes('Visible','off','DrawMode','fast','Parent',st.fig,'DeleteFcn',DeleteFcn,...
		'YDir','normal','ButtonDownFcn',...
		['if strcmp(get(gcf,''SelectionType''),''normal''),myspm_orthviews(''Reposition'');',...
		'myspm_orthviews(''set_pos2cm'');',...
		'elseif strcmp(get(gcf,''SelectionType''),''extend''),myspm_orthviews(''Reposition'');',...
		'myspm_orthviews(''set_pos2cm'');',...
		'myspm_orthviews(''context_menu'',''ts'',1);end;']);
...
for i=valid_handles(1:24),
	...
	DeleteFcn = ['myspm_orthviews(''Delete'',' num2str(i) ');'];
...
for i = valid_handles(arg1),
	...
	ok=1;
	eval('imgt  = (myspm_slice_vol(st.vols{i},TM,TD,st.hld))'';','ok=0;');
	eval('imgc  = (myspm_slice_vol(st.vols{i},CM,CD,st.hld))'';','ok=0;');
	eval('imgs  = (myspm_slice_vol(st.vols{i},SM,SD,st.hld))'';','ok=0;');
...
pluginpath = fullfile(spm('Dir'),'myspm_orthviews');
if isdir(pluginpath)
	pluginfiles = dir(fullfile(pluginpath,'spm_ov_*.m'));
	if ~isempty(pluginfiles)
		addpath(pluginpath);
		% fprintf('myspm_orthviews: Using Plugins in %s\n', pluginpath);
...
item1_1   = uimenu(item1,      'Label','Full Volume',   'Callback','myspm_orthviews(''context_menu'',''zoom'',6);', 'Checked','on');
item1_2   = uimenu(item1,      'Label','160x160x160mm', 'Callback','myspm_orthviews(''context_menu'',''zoom'',5);');
item1_3   = uimenu(item1,      'Label','80x80x80mm',    'Callback','myspm_orthviews(''context_menu'',''zoom'',4);');
item1_4   = uimenu(item1,      'Label','40x40x40mm',    'Callback','myspm_orthviews(''context_menu'',''zoom'',3);');
item1_5   = uimenu(item1,      'Label','20x20x20mm',    'Callback','myspm_orthviews(''context_menu'',''zoom'',2);');
item1_6   = uimenu(item1,      'Label','10x10x10mm',    'Callback','myspm_orthviews(''context_menu'',''zoom'',1);');

%contextsubmenu 2
item2     = uimenu(item_parent,'Label','Crosshairs');
item2_1   = uimenu(item2,      'Label','on', 'Callback','myspm_orthviews(''context_menu'',''Xhair'',''on'');','Checked','on');
item2_2   = uimenu(item2,      'Label','off', 'Callback','myspm_orthviews(''context_menu'',''Xhair'',''off'');');

%contextsubmenu 3
item3     = uimenu(item_parent,'Label','Orientation');
item3_1   = uimenu(item3,      'Label','World space', 'Callback','myspm_orthviews(''context_menu'',''orientation'',2);');
item3_2   = uimenu(item3,      'Label','Voxel space', 'Callback','myspm_orthviews(''context_menu'',''orientation'',1);','Checked','on');

%contextsubmenu 4
item4     = uimenu(item_parent,'Label','Interpolation');
item4_1   = uimenu(item4,      'Label','NN',    'Callback','myspm_orthviews(''context_menu'',''interpolation'',3);');
item4_2   = uimenu(item4,      'Label','Bilin', 'Callback','myspm_orthviews(''context_menu'',''interpolation'',2);','Checked','on');
item4_3   = uimenu(item4,      'Label','Sinc',  'Callback','myspm_orthviews(''context_menu'',''interpolation'',1);');

%contextsubmenu 5
% item5     = uimenu(item_parent,'Label','Position', 'Callback','myspm_orthviews(''context_menu'',''position'');');
...
item6_1_1_2 = uimenu(item6_1_1,  'Label','manual',     'Callback','myspm_orthviews(''context_menu'',''window'',1);');
item6_1_2   = uimenu(item6_1,    'Label','global');
item6_1_2_1 = uimenu(item6_1_2,  'Label','auto',       'Callback','myspm_orthviews(''context_menu'',''window_gl'',2);');
item6_1_2_2 = uimenu(item6_1_2,  'Label','manual',     'Callback','myspm_orthviews(''context_menu'',''window_gl'',1);');
item6_2     = uimenu(item6,      'Label','Swap image', 'Callback','myspm_orthviews(''context_menu'',''swap_img'');');

%contextsubmenu 7
item7     = uimenu(item_parent,'Label','Blobs');
item7_1   = uimenu(item7,      'Label','Add blobs');
item7_1_1 = uimenu(item7_1,    'Label','local',  'Callback','myspm_orthviews(''context_menu'',''add_blobs'',2);');
item7_1_2 = uimenu(item7_1,    'Label','global', 'Callback','myspm_orthviews(''context_menu'',''add_blobs'',1);');
item7_2   = uimenu(item7,      'Label','Add colored blobs');
item7_2_1 = uimenu(item7_2,    'Label','local',  'Callback','myspm_orthviews(''context_menu'',''add_c_blobs'',2);');
item7_2_2 = uimenu(item7_2,    'Label','global', 'Callback','myspm_orthviews(''context_menu'',''add_c_blobs'',1);');
item7_3   = uimenu(item7,      'Label','Add colored image');
item7_3_1 = uimenu(item7_3,    'Label','local',  'Callback','myspm_orthviews(''context_menu'',''add_c_image'',2);');
item7_3_2 = uimenu(item7_3,    'Label','global', 'Callback','myspm_orthviews(''context_menu'',''add_c_image'',1);');
...
case 'xhair',
	myspm_orthviews('Xhairs',varargin{2});
...
case 'orientation',
	if varargin{2} == 2,
		myspm_orthviews('Space');
	else,
		myspm_orthviews('Space',1);
	end;
	cm_handles = get_cm_handles;
	for i = 1:length(cm_handles),
...
case 'window',
	...
	if varargin{2} == 2,
		myspm_orthviews('window',current_handle);
	else
		myspm_orthviews('window',current_handle,spm_input('Range','+1','e','',2));
	end;
...
case 'add_blobs',
	...
	for i = 1:length(cm_handles),
		...
		item7_3_1 = uimenu(c_handle,'Label','local','Callback','myspm_orthviews(''context_menu'',''remove_blobs'',2);');
		if varargin{2} == 1,
			item7_3_2 = uimenu(c_handle,'Label','global','Callback','myspm_orthviews(''context_menu'',''remove_blobs'',1);');
		end;
	end;
	redraw_all;
...
case 'add_c_blobs',
	...
	for i = 1:length(cm_handles),
		...
		item7_4_1   = uimenu(ch_c_handle(2),'Label',c_names{c},'ForegroundColor',colours(c,:),...
			'Callback','c = get(gcbo,''UserData'');myspm_orthviews(''context_menu'',''remove_c_blobs'',2,c);',...
			'UserData',c);
		if varargin{2} == 1,
			item7_4_2 = uimenu(ch_c_handle(1),'Label',c_names{c},'ForegroundColor',colours(c,:),...
				'Callback','c = get(gcbo,''UserData'');myspm_orthviews(''context_menu'',''remove_c_blobs'',1,c);',...
				'UserData',c);
		end;
	end;
	redraw_all;
...
case 'add_c_image',
	...
	for i = 1:length(cm_handles),
		...
		%set(ch_c_handle,'Visible',on');
		item7_4_1 = uimenu(ch_c_handle(2),'Label',c_names{c},'ForegroundColor',colours(c,:),...
			'Callback','c = get(gcbo,''UserData'');myspm_orthviews(''context_menu'',''remove_c_blobs'',2,c);','UserData',c);
		if varargin{2} == 1
			item7_4_2 = uimenu(ch_c_handle(1),'Label',c_names{c},'ForegroundColor',colours(c,:),...
				'Callback','c = get(gcbo,''UserData'');myspm_orthviews(''context_menu'',''remove_c_blobs'',1,c);',...
				'UserData',c);
...
for i = 1:length(valid_handles(1:24)),
	if isfield(st.vols{i}.ax{1},'cm')
		set(findobj(st.vols{i}.ax{1}.cm,'UserData','pos_mm'),...
			'Label',sprintf('mm:  %.1f %.1f %.1f',myspm_orthviews('pos')));
		pos = myspm_orthviews('pos',i);
