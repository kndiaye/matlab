...
for Order = OPTIONS.SourceModel % Compute gain matrices for each requested source models    (-1 0 1)
    ...
    for j = 1:(OPTIONS.ImageGridBlockSize):nv,  
        ...
        elseif nargout>1
            if ~exist('GG', 'var')
                if OPTIONS.ApplyGridOrient
                    GG = NaN*zeros(length(OPTIONS.Channel),nv);
                else
                    GG = NaN*zeros(length(OPTIONS.Channel),Dims*nv);
                end
            end
            if MEG 
                GG(MEGndx,ndx) = G(MEGndx,:);   
            end
            if EEG
                GG(EEGndx,ndx) = G(EEGndx,:);   
            end
...
elseif nargout == 1
    ...
    if OPTIONS.ImageGridBlockSize < nv & ~isempty(OPTIONS.HeadModelFile)
