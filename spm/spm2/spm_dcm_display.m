...
% FORMAT spm_dcm_display(xY,a,c,u,M,U,img)
...
% beta  - image contrast ( 0 < beta < 1 )
% img   - image to plot on
% markerstyle - Marker size
% markerstyle2 - Marker style
...
if n < 7; beta=.24; else,   beta = varargin{7};	 end
if n < 8; img = []; else, img=varargin{8};	end
if isempty(img)
    fullfile(spm('Dir'),'canonical','single_subj_T1.mnc'); 
end
markerstyle = {'MarkerSize',64,'Marker', '.'};   
if n >= 9; markerstyle=[markerstyle varargin{9}]; end
markerstyle2 = {'MarkerSize',4,'Marker', '.'};   
if n >= 10; markerstyle2=[markerstyle2 varargin{10}]; end
if n < 11; imgori='sa';  else    imgori=varargin{11}; end
...
%---------------------------------------------------------------------------
if ~exist(img, 'file')
    img = fullfile(spm('Dir'),'canonical','single_subj_T1.mnc');
end
try
    V = spm_vol(img);
catch
...
t1        = size(colormap,1)*((1 - beta) + beta*t1/max(t1(:)));
