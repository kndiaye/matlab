		...
		try
			if iscell(P{j}),			
				...
				V{j} = subfunc1(P{j});
			            ...
			            % Handling of files being moved around...
            persistent pwd
            if ~isempty(pwd)
            end
warning('Not found: %s\n Try changing filesep...', P{j})
			P{j} = translocate(P{j});            
			if iscell(P{j}),			
				V{j} = subfunc2(P{j});
			else
			...
			warning('ok! filesep has been changed in %s', P{j})
		end
	end;
else
	try
        ...
        V = subfunc1(translocate(P));
        warning('ok! filesep has been changed in %s', P)
    end
end;
return;

function Q=translocate(P)
% from Linux to Win & vice-versa.
if isunix
    Q = strrep(P, '\', filesep);
else
    Q = strrep(P, '/', filesep);
