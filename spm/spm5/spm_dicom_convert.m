...
%
% This version of spm_dicom_convert has been hacked so that it can deal
% with MOSAIC type functional images whose headers have been stripped of
% the CSA header information. CSA refers to private DICOM header fields
% used by SIEMENS, which can go missing when images are archived to a DICOM
% server. The result is that SPM's DICOM import produces volumes that are
% very large, very thin, and contain a collection of seperate slices of
% brain. Now it seems to work for the images I have tested it with, but it
% may not work for yours. In particular, it is important to carefully check
% that your images are correctly oriented (not right-left flipped for
% example).
%
% I have also changed the default setting for the output file format
% from 'img' to 'nii'. 
%
% SWR 2008-08-26
% Sebastian Rieger, LabNIC, University of Geneva



if nargin<2, opts = 'all'; end;
if nargin<3, root_dir='flat';end;
if nargin<4, format='nii';end; % Changed this from 'img' to 'nii'.
                               % SWR 2008-08-26
...
for i=1:length(hdr),
    ...
    try
        ...
        % This will fail if the CSA header information is missing from a
        % mosaic type DICOM file, so I have put it in a "try" statement.
        % SWR 2008-08-20
    catch
        dim(3) = (nc*nr) / ((max(hdr{i}.AcquisitionMatrix))^2);
        % This is an attempt to guess the number of images in the mosaic if
        % it cannot be read from CSA header information. Instead, the
        % overall dimensions of the mosaic in pixels and the size of the
        % acquisition matrix ("Columns", "Rows", and "AcquisitionMatrix"
        % in the DICOM header) are used.
        % SWR 2008-08-20
    end
    ...
    try
        ...
        % This will fail if CSA header information is missing from  a 
        % mosaic type DICOM file, so I have put it into a "try" statement.
        % SWR 2008-08-20
        ...
        % Ideally, another way to check orientation (without relying on 
        % CSA header information) should be implemented here. Until then,
        % image orientation must be carefully checked manually!
        % SWR 2008-08-20
...
% This is the patched version of SPM's select_mosaic_images function. The
% original (see below) fails to recognise mosaic type DICOM files if the
% CSA image header info is missing. SWR 2008-08-26
mosaic   = {};
standard = {};
for i=1:length(hdr),
    if (~checkfields(hdr{i},'ImageType','CSAImageHeaderInfo') ||...
            ...
            && (isempty(strfind(hdr{i}.ImageType, 'MOSAIC')))
            % SWR: This last line identifies (at least some) mosaics even
            % when there is no CSA image header present. SWR
...
%{
This is the original version of SPM's select_mosaic_images function:
%_______________________________________________________________________
function [mosaic,standard] = (hdr)
mosaic   = {};
standard = {};
for i=1:length(hdr),
    if ~checkfields(hdr{i},'ImageType','CSAImageHeaderInfo') ||...
