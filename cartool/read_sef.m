function [eeg]=read_sef(seffile)
%read_sef() - reads SEF file (Cartool)
%   [eeg]=read_sef(seffile)
...
eeg.Version             = char(fread(fid,4,'uint8')'); % fread(fid,1,'int');
...
eeg.TSefChannelName = reshape(char(fread(fid,8*eeg.NumElectrodes,'char')),8,eeg.NumElectrodes)' ;

eeg.data = reshape(fread(fid,eeg.NumElectrodes*eeg.NumTimeFrames,'float'),eeg.NumElectrodes,eeg.NumTimeFrames);
