function [GD,TC, GDbis, PCL]=geod_cluster2(VC, Coord, SS, varargin)

% function [GD, TC, GDbis, PCL] = geod_cluster2(VC , Coord, SS [, options])
...
while nsv<nv,       % tant qu'on n'a pas visite toutes les sources (on affinera apres, 
 ...
 %   nsv2=nsv;                                
 if debug
   disp(sprintf('Main loop %d/%d', nsv, nv))
 end                  
 for k=1:K 
      if debug
	disp(sprintf('visiting seed %d', k))
      end
        if (d<size(PH(k,:),2))
        ...
        for s=1:size(PH{k,d+1},2)
            ...
            for v = 1:size(VC{ss},2)     % la on regarde chaque voisin de notre source S
                 if debug
		   disp(sprintf('Voisin : %d de source %d', v,s))
		 end
		 V = VC{ss}(v);
                de=0;
                for i=1:3
