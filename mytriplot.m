...
% 
% Use as
%   triplot(pnt, dhk, value, mode, levels)
% This will make a plot of value on the surface described by triangles
% dhk with vertices pnt. The matrix dhk can be [], in which case a
...
if isempty(dhk) & ~strcmp(mode, 'nodes')
  % no triangulation was specified but a triangulation is needed
  mpnt=mean(pnt);
  pnt=pnt-repmat(mpnt,size(pnt,1),1)
...
if isequal(mode, 'contour') | isequal(mode, 'contour_bw') | isequal(mode, 'contour_rb')
...
if length(val)==size(pnt,1)
  hs = patch('Vertices', pnt, 'Faces', dhk, 'FaceVertexCData', val, 'FaceColor', 'interp');
else
  hs = patch('Vertices', pnt, 'Faces', dhk, 'CData', val, 'FaceColor', 'flat');
end
set(hs, 'EdgeColor', 'none');
