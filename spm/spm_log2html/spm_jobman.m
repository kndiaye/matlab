...
if isfield(c,'prog')
    ...
    if 0
        ...
        %prog_log=[prog_infos.parentage{end} '_log'];
        %if exist(prog_log) > 1
        try
            spm_log2html(prog,val);
            disp(sprintf('Generating log on %s',c.name));
        catch
            disp(sprintf('Error while generating log on %s',c.name));
        end
        %else
        %   prog_msg=sprintf('No function found for generating log on %s. Try making a: %s',c.name, prog_log);
        %end
        %disp(prog_msg);
        spm('Pointer');
    end;
