...
% SUBFUNCTIONS:
%
% FORMAT so = dstrtrim(si)
% Double tailed strtrim - strips blanks from both ends of string matrix
%
...
uicontrol(F,'Style','PushButton','Tag','CDhome',...
	...
	'ForegroundColor','r',...
	'CallBack',['spm_get(''cd'',''',strtrim(LastDirs(2,:)),''')'],...
...
if size(Dirs,1)
    for i = 1:size(Dirs,1)
	h = text(0,y,strtrim(Dirs(i,:)),...
		'Parent',hAxes,...
		'Tag','DirName',...
		'FontWeight','bold','Color','r',...
		'UserData',strtrim(Dirs(i,:)),...
...
if strcmp(Cend,'front')
	if size(Fnames,1)==1 | size(Fnames,2)<len
		...
		varargout = {strtrim(Fnames),[1:size(Fnames,1)]'};
		return
	end
...
elseif strcmp(Cend,'both')
	...
	for i = 1:size(hSpecs,1)
        ...
        %KND:
        FSpecs   = strvcat(FSpecs,hSpecs(i,:));
...
%-----------------------------------------------------------------------
set(findobj(F,'Tag','P'),'UserData',strtrim(P));
...
if isempty(WDir), WDir=pwd; else, WDir=strtrim(WDir); end
...
while ~( Done | (nP>=n.mx) )
	...
	str = []; while isempty(str)
		str=dstrtrim(input(sprintf('  %3d  : ',nP+1),'s'));end
...
for i=1:prod(size(cpath))
	...
	while length(cpath{i})>=2 & length(findstr(cpath{i},dubsep))
		if (ispc & strncmp(dubsep, cpath{i},2)) 
            %initial '\\' is used for network drive in Windows' world
            cpath{i}= [sepchar , strrep(cpath{i},dubsep,sepchar)]; 
        else
            cpath{i}=strrep(cpath{i},dubsep,sepchar); 
        end
    end
...
function [T]=strtrim(S);
% strtrim - remove blank spaces
...
elseif ischar(S)
    T=fliplr(deblank(fliplr(deblank(S))));
end
