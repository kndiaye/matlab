...
%
%KND additions:
%   'channames' = ['on'|'off'] display channel names
%   'chancb'    = ['plot1'|...] Callback function for each channel trace.
%                 If 'plot1' (default): When clicked the single trace is
%                 plotted.
%   
  ...
  for P=0:datasets-1, %  for each data epoch
    ...
    for c=1:chans, %%%%%%%% for each data channel %%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if ~ISSPEC % -/+ plot, normal case (e.g., not spectra), plot data trace           
            figure(curfig);
            plot(x,data(c,1+P*g.frames:1+P*g.frames+g.frames-1), ...
                      'color', tmpcolor{:});   
            ymn = min([ymax ymin]);
            ymx = max([ymax ymin]);
            if g.ydir == -1
