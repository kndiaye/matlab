...
str  = { 'overlays...','slices','sections','render', 'colin27'};
tstr = { 'overlay filtered SPM on another image: ',...
	 '3 slices / ','ortho sections / ','render', 'sections from colin27''s canonical brain'};
tmp  = { 'spm_transverse(''set'',xSPM,hReg)',...
	...
	['spm_render(	struct(	''XYZ'',	xSPM.XYZ,',...
				...
				'''mat'',	xSPM.M,',...
				'''dim'',	xSPM.DIM))'],...
     'spm_sections(xSPM,hReg,fullfile(spm(''dir''),''canonical'',''single_subj_T1.mnc''))'};
     % 'spm_sections_colin27(xSPM,hReg)'};
