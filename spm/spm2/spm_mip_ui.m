...
if DIM(3) == 1
	...
	hX1r   = text(Po(1)+xyz(2),Po(2)+xyz(1),'<',...
			...
			'Tag','hX1r',...
			'ButtonDownFcn','spm_mip_ui(''MoveStart'')');
	hX2r   = text(Po(1)+xyz(2),Po(3)-xyz(3),'<',...l
			'Color','r','Fontsize',16,...
			'Tag','hX2r',...
			'ButtonDownFcn','spm_mip_ui(''MoveStart'')');
...
uimenu(h,'Separator','off','Label','goto nearest local maxima',...
	...
	'Interruptible','off','BusyAction','Cancel','Enable',str);
% KND:
uimenu(h,'Separator','off','Label','goto maximum in a sphere',...
	'CallBack',['spm_mip_ui(''Jump'',',...
		'get(get(gcbo,''Parent''),''UserData''),''sphmax'');'],...
	'Interruptible','off','BusyAction','Cancel','Enable',str);
...
%KND if many MIP windows...
if length(h)>1
    h=h(strmatch('Graphics',get(cell2mat(get(h, 'Parent')), 'Tag')));
    h=h(end);
end
...
case 'sphmax'
	str       = 'maxima in sphere';
    sph_r=inputdlg('Radius of the sphere?','Max in sphere',1,{'12'});
    if isempty(sph_r)
        return
    end
    sph_r=str2num(sph_r{1});
    
    iM        = inv(MD.M);
	XYZvox    = iM(1:3,:)*[MD.XYZ; ones(1,size(MD.XYZ,2))];
    [d] = spm_XYZreg('Edist',oxyz,MD.XYZ);
    i=find(d<=sph_r);
    XYZvox    = iM(1:3,:)*[MD.XYZ(:,i); ones(1,length(i))];    	

	i_max      = i(find(MD.Z(i)==max(MD.Z(i))));
    if ~isempty(i_max)
        xyz       = MD.XYZ(:,i_max);
        d         = sqrt(sum((oxyz-xyz).^2));
    else
...
try
    fprintf(' -> voxel #[%d %d %d]', round(inv(MD.M)*[xyz; 1]));
end
fprintf('\n');
