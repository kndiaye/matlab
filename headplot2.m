...
function [] = headplot(values,arg1,p1,v1,p2,v2,p3,v3,p4,v4,p5,v5,p6,v6)
...
if isstr(values)
    ...
    if strcmp(values,'setup')
        ...
        if isstr(eloc_file)
            ...
            if strcmp(loctype,'spherical')
                [a,b,c, s]=textread(eloc_file, '%d %f %f %s');
                A=[a b c];
                ElectrodeNames=char(s);
            elseif strcmp(loctype,'cartesian')
                [a,x,y,z, s]=textread(eloc_file, '%d %f %f %f %s');
                A=[a x y z];
                ElectrodeNames=char(s);
            else
                error(['headplot: unknown electrode location file type.\n']);
            end
            if 0 % OLD THING WITH dot in EEGnames etc !
                fid = fopen(eloc_file);
                if fid == -1
    ...
    %KND
    POS=rotation(POS, [0,0,0], [0 1 0] , -30);
