function [varargout]=spm_render2(dat,brt,rendfile)
...
if isstruct(dat) & isfield(dat, 'rend')
    rend=dat.rend;
    mxmx=dat.mxmx;
    mnmn=dat.mnmn; 
    clear dat;
...
if ~finite(brt),
    ...
    for i=1:length(rend),
        ren = rend{i}.ren;
        ren = repmat(ren, [ 1 1 3]);
        % KND:
        rgb=[];
        for j=1:length(rend{i}.data),
            X = rend{i}.data{j}/(mxmx-mnmn)-mnmn;
            col = Colors(j,:);
            if isempty(rgb)
                rgb=zeros([size(X) 3]);
            end
            rgb = rgb + repmat(reshape(col, [1,1,3]), size(X)).*repmat(X, [1 1 3]);
        end
        rgb= rgb*1.3+0.8*ren; 
        % rgb = rgb.*ren + ren.*repmat(all(rgb==0,3), [1 1 3]); 
        rgb=min(rgb,1);
        rgb=max(rgb,0);
