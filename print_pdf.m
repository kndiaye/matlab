...
% $http://www.mathworks.com/matlabcentral/fileexchange/23056?controller=file_infos&download=true
...
if nargin<1
    name = get(fig, 'Name');
    if isempty(name)
        name=['fig_' num2str(fig) ];        
    end
    if exist(name, 'file')
        name = [name '_' datestr(now,30)];
    end
end
...
% Test if any transparency in picture
transparency = [] ; 
for i=[findobj(fig)]'
    if isequal(get(i, 'Visible'), 'on') && ( ...
            (isfield(get(i), 'FaceAlpha') && get(i, 'FaceAlpha')<1) || ...
            (isfield(get(i), 'EdgeAlpha') && get(i, 'EdgeAlpha')<1) )
        transparency =[transparency, i]; 
    end
end
if ~isempty(transparency)
    % If any, create another temp EPS file
    tmp_nam_tr = [tempname '.eps']
    print(fig, '-depsc2', '-opengl',  tmp_nam_tr);
    % Fix the line styles
    fix_lines(tmp_nam_tr);
end
...
cmd = '';
if ispc
    persistent default_location executable
    if isempty(default_location)
        if exist([mfilename, '.mat'], 'file')
            load([mfilename, '.mat']);
            if isequal(questdlg(sprintf('Use: %s\%s ?', default_location, executable),'GhostScript configuration', 'Yes'), 'No')
                default_location = [];
                executable = [];
            end            
        else
    ...
    if ~exist(fullfile(default_location,executable), 'file')
        [executable, default_location] = uigetfile('gswin32c.exe', 'Pick the GhostScript Viewer',default_location);
        if isequal(executable,0)
            return
        end
        save([mfilename('fullpath'), '.mat'], 'default_location','executable');
    end
else
...
ver_num = 0;
cmd = fullfile(default_location,executable);
