...
try
    ...
    if strcmp(check_spm_design_matrix, 'yes')
        ...
        if isfield(spmMat , 'xX')
            ...
            if not(isempty(spmMat.xCon))
                for iCon=1:length(spmMat.xCon)
                    if isequal('T', spmMat.xCon(iCon).STAT)
                        spmMat.xX.X = [ spmMat.xX.X spmMat.xX.X(:,1:numRefFunc)*spmMat.xCon(iCon).c ];
                        spmMat.xX.name = [spmMat.xX.name {spmMat.xCon(iCon).name}];
                    end
                end
            end
            numRefFunc = length(spmMat.xX.name);
    ...
    if strcmp(lower(check_regressors), 'yes')
        ...
        if strcmp(lower(flag_refFunc), 'session_specific') & ~isempty(data_sessionNumber)
            ...
            for ii = 1:numRefFunc
                ...
                if isempty(firstBracket) || isempty(endBracket)
                    warning('Contrasts appear in all sessions')
                    currentNumber = data_sessionNumber;%:length(spmMat.Sess);
                else
