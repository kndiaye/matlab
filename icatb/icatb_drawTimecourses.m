...
plotH = icatb_uicontrol('parent', timecourse_handle, 'units', 'normalized', 'style', 'pushbutton', 'string', ...
    'Plot', 'position', pos, 'callback', {@plotTC_expandedMode, timecourse_handle}, 'tag', 'plot_button');
...
if ~isempty(figureData.modelTimecourse)
    ...
    referenceString = repmat({'Reference Function'},ncols,1);
    
    %if length(figureData.sortParameters.refInfo.selectedRegressors)==1
    referenceString = cellstr(figureData.sortParameters.refInfo.selectedRegressors.name);
    %end
    ...
    for i = 1:ncols
        modelPlotHandle(i) = plot(modelTimecourse(:, i), 'y:', 'parent', axisH, 'tag', 'regressor');
        set(modelPlotHandle(i), 'ButtonDownFcn', [ ...
            'set(findobj(gcbf, ''tag'', ''regressor'', ''Type'', ''line''),''Color'',''y'',''LineWidth'',1,''LineStyle'', '':'');' ...
            'set(gcbo,''LineWidth'',2,''LineStyle'', '':'',''Color'', ''y'')' ...
            ]); 
        legendString{length(legendString) + 1} = referenceString{i};
        hold on;
    end
