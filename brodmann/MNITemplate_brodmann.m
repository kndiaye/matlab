...
try
    ...
    addpath(fullfile(USBDIR, 'matlab/ctf2tal/'))
    addpath(fullfile(USBDIR, 'matlab/stormvisa'))
    addpath(fullfile(USBDIR, 'matlab/brodmann'))
end

load(fullfile(HOMEDIR, 'mtoolbox/brainstorm/MNITemplate/MNI_braincereb_small_tess_CTF'))
fv.vertices=Vertices{end}';
try
    fv.vertconn=VertConn{end};
catch
end
clear Vertices;
fv.faces=Faces{end};
...
S=load(fullfile(HOMEDIR, 'mtoolbox/brainstorm/MNITemplate/MNI_subjectimage'), 'talCS', 'SCS')
...
% Apply Talairach nomalization to vertex locations
%1)Load Talairach fiducials and bounding box from MRI file
% 2) Apply global Talairach transform
[fv2.vertices] = scs2tal(S.talCS.scsCubeFiducial/1000, S.talCS.cortexTess,fv.vertices', S.talCS.Type);

fv2.vertices=fv2.vertices'*1000;
v2=fv2.vertices;

figure(1)
clf
view_cortex(fv2.faces,v2,'FaceColor','none', 'EdgeColor', 'k' , 'FaceAlpha', .5)
hold on
view_cortex(fv2.faces,mni2tal(v2), 'FaceColor','none', 'EdgeColor', 'g')
...
% Same story for new (target)tessellation 
load(newTess,'Faces','Vertices')
newFaces = Faces{iGrid};
newVertices = Vertices{iGrid};

MRI = load(newMRI,'talCS'); 
[newVertices] = scs2tal(MRI.talCS.scsCubeFiducial/1000, MRI.talCS.cortexTess,newVertices, MRI.talCS.Type);
